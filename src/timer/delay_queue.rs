...
use super::{Error, Delay};
use super::clock::now;
use super::wheel::{self, Wheel};
use super::timer::Handle;
...
impl<T> DelayQueue<T> {
    ...
    fn poll_idx(mut self: Pin<&mut Self>, lw: &LocalWaker) -> Poll<Result<Option<usize>, Error>> {
        use self::wheel::Stack;

        let expired = {
            let Self { expired, slab, .. } = &mut *self;
            expired.pop(slab)
        };
        ...
        loop {
            if let Some(mut delay) = self.as_mut().delay() {
                if !delay.is_elapsed() {
                    ready!(delay.as_mut().poll(lw)?);
                }
            ...
            let idx = {
                let Self { wheel, poll, slab, .. } = &mut *self;
                wheel.poll(poll, slab)
            };
            ...
            let deadline = match self.as_mut().wheel.poll_at() {
                Some(poll_at) => {
    ...
    fn delay<'a>(self: Pin<&'a mut Self>) -> Option<Pin<&'a mut Delay>> {
        unsafe {
            match &mut Pin::get_mut_unchecked(self).delay {
                Some(delay) => Some(Pin::new_unchecked(delay)),
                None        => None,
            }
        }
    }
...
impl<T> Stream for DelayQueue<T> {
    ...
    fn poll_next(mut self: Pin<&mut Self>, lw: &LocalWaker) -> Poll<Option<Self::Item>> {
        let item = ready!(self.as_mut().poll_idx(lw)?)
            .map(|idx| {
                let data = self.as_mut().slab.remove(idx);
